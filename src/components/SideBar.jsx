  ...
  return (
    <aside className="bg-white text-[#666666] flex-1 min-w-64 flex flex-col overflow-auto items-center h-screen rounded-tr-[45px]  rounded-br-[45px] border-[rgba(0, 0, 0, 0.30)] border-r">
        ...
        {navLinks.map((link) => (
          <Link href={link?.path ? link?.path : '/'}>
            <li
              ...
              key={link.id}
              className={`transition-all ease-in-out duration-300 p-[16px] text-xl cursor-pointer flex items-center gap-[15px] font-semibold  hover:bg-[rgba(190, 18, 60, 0.10)] ${
                active === link.id
                  ? 'bg-[#BE123C1A]/10 text-[#BE123C]'
                  : 'text-[#666666]'
              }`}
            >
      ...
      <section className="bg-[#F8E7EB66]/40 rounded-[20px] pt-[42px] p-5 flex justify-center flex-col items-center gap-2 mx-[28px]">
        <p className="text-[#333333CC]/80 font-semibold text-base">
          Play movie quizes and earn free tickets
        </p>
        <p className="text-[#666666] text-sm font-[500]">
          50k people are playing now
        </p>
        <button className="text-[#BE123C] font-[500] text-sm p-2 flex w-[80%] justify-center items-center  bg-[#BE123C33]/20 rounded-[30px]">
          Start playing
        </button>
      </section>
