...
const Index = () => {
  const { movieLoading, movieError, movie } = useSelector(
    (state) => state.movies
  );
                ...
                <div className="flex justify-between mb-12">
                  <div className="flex flex-wrap font-semibold gap-3 items-center text-base mt-4 text-[#404040]">
                    <h1
                      ...
                      className="m-0 text-xl md:text-2xl w-auto font-bold"
                    >
                      {movie?.title},
                    </h1>
                    ...
                    <ul className="flex items-center gap-2 text-[#B91C1C] w-full md:w-auto flex-wrap text-[15px] font-[500]">
                      {movie?.genres?.map((genre) => (
                        <li
                          className="shadow-lg border border-[#F8E7EB]  px-4 flex items-center justify-center rounded-xl"
                          key={genre.id}
                        >
                          {genre?.name}
                        </li>
                  ...
                  <article className="col-span-2 flex flex-col gap-3">
                    <div className="flex  gap-4">
                      <p
                        ...
                        className="text-[#333333] text-[14px] md:text-base font-semibold"
                      >
                        {movie?.overview}
                      </p>
                    </div>
                    <div className="flex flex-wrap gap-4">
                      ...
                      <p className="text-[#B91C1C] text-[14px] md:text-basefont-semibold">
                    ...
                    <div className="flex flex-wrap gap-4">
                      ...
                      <ul className="flex items-center gap-2 text-[#B91C1C] text-[14px] md:text-base font-[500]">
                        {movie?.spoken_languages?.map((lang) => (
                          <p
                            className="border border-[#F8E7EB]  px-4 flex items-center justify-center rounded-lg"
                            key={lang.id}
                          >
                            {lang?.english_name}
                          </p>
                    ...
                    <div className="flex flex-wrap gap-2">
                      <p className="font-semibold flex text-[14px] md:text-base">
                        Tag line :
                      </p>
                      <p className="text-[#B91C1C] flex-wrap text-[14px] md:text-base font-semibold">
                        {movie?.tagline}
                      </p>
                    </div>
                  </article>
                  <article className="col-span-1">
                    <div className="flex flex-col gap-2">
                      <button className="bg-[#BE123C] py-1 md:py-2 px-2 md:px-4  gap-2 rounded-xl text-white flex justify-center items-center">
                        <Image
                          src="/assets/images/Two Tickets.svg"
                          alt="Star"
                          width={10}
                          height={10}
                          layout="intrisic"
